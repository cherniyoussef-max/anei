  ...
  return (
    <>
      ...
      <section className="section section-tint"><div className="container">
        ...
        <div className="course-grid compact-grid">{courses.slice(0, 3).map((course, index) => <article className="course-card course-card-human" key={course.slug}><span className="course-index" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span><div className="course-card-body"><span className="course-category-label">{categoryLabels[course.category]?.[locale] ?? course.category}</span><h3>{course.title[locale]}</h3><p>{course.description[locale]}</p><div className="meta-row"><span><Icon name="clock" size={15}/>{course.duration}</span><span><Icon name="calendar" size={15}/>{formatDate(course.startDate, locale)}</span></div><div className="card-footer"><strong>{formatPrice(course.price, locale)}</strong><Link className="text-link" href={`/${locale}/formations/${course.slug}`}>{c.actions.details}<Icon name="arrow" size={16}/></Link></div></div></article>)}</div>
      </div></section>

      <section className="section human-story-section"><div className="container human-story-grid">
        <div className="human-story-photo"><Image src="/media/anei-learning-story-v2.webp" alt={ar ? "شخصان يتعاونان حول أنشطة ومواد تعليمية" : "Deux personnes collaborant autour d’activités et de supports pédagogiques"} width={1200} height={900} sizes="(max-width: 900px) 100vw, 48vw"/><div className="human-story-label"><Icon name="spark" size={17}/><span>{ar ? "تعلم يركز على الإنسان" : "Une pédagogie centrée sur l’humain"}</span></div></div>
