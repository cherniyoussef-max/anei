...
function googleCalendarLink(title: string, startAt: string, endAt: string, details: string) {
  const stamp = (iso: string) => iso.replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
  const params = new URLSearchParams({ action: "TEMPLATE", text: title, dates: `${stamp(startAt)}/${stamp(endAt)}`, details });
...
export function AppointmentBooking({ locale, slots, rescheduleId }: { locale: Locale; slots: LearnerSlot[]; rescheduleId?: string }) {
  ...
  const [confirmed, setConfirmed] = useState<{ slot: LearnerSlot; meetingUrl: string } | null>(null);
  ...
  async function confirm() {
    ...
    if (response.ok) {
      const body = await response.json().catch(() => null) as { meetingUrl?: string } | null;
      setState("done");
      setConfirmed(body?.meetingUrl ? { slot: selected, meetingUrl: body.meetingUrl } : null);
      setSelected(null);
      router.refresh();
      return;
    }
  ...
  if (!slots.length) return <section className="appointment-booking-empty" aria-labelledby="booking-title"><CalendarDays size={24} strokeWidth={1.75} /><div><h2 id="booking-title">{ar ? "لا توجد أوقات متاحة حاليًا" : "Aucun créneau disponible actuellement"}</h2><p>{ar ? "لم يفتح فريق المتابعة أوقات حجز خلال الأسابيع الستة القادمة." : "L’équipe de suivi n’a ouvert aucun créneau dans les six prochaines semaines."}</p></div></section>;
  return <section className="appointment-booking" aria-labelledby="booking-title"><div className="appointment-booking-heading"><h2 id="booking-title">{rescheduleId ? (ar ? "إعادة جدولة الموعد" : "Replanifier le rendez-vous") : (ar ? "حجز موعد" : "Prendre rendez-vous")}</h2><p>{ar ? "اختر تاريخًا ثم وقتًا متاحًا. يُعاد التحقق من الوقت عند التأكيد." : "Choisissez une date puis un créneau. La disponibilité est revérifiée à la confirmation."}</p>{rescheduleId ? <Link href={`/${locale}/dashboard/rendez-vous`}>{ar ? "إلغاء إعادة الجدولة" : "Annuler la replanification"}</Link> : null}</div>
    ...
    {state === "done" ? <div className="appointment-feedback is-success" role="status">
      <p>{ar ? "تم حجز الموعد وتحديث قائمتك." : "Rendez-vous confirmé et liste mise à jour."}</p>
      {confirmed ? <div className="appointment-confirmed-links">
        <a href={confirmed.meetingUrl} target="_blank" rel="noreferrer" className="student-primary-action">{ar ? "رابط الاجتماع" : "Lien de la réunion"}</a>
        <a href={googleCalendarLink(ar ? "موعد ANEI" : "Rendez-vous ANEI", confirmed.slot.startAt, confirmed.slot.endAt, confirmed.meetingUrl)} target="_blank" rel="noreferrer" className="student-secondary-action">{ar ? "إضافة إلى تقويم Google" : "Ajouter à Google Agenda"}</a>
      </div> : null}
    </div> : state === "conflict" ? <p className="appointment-feedback is-error" role="alert">{ar ? "حجز شخص آخر هذا الوقت. اختر وقتًا آخر." : "Ce créneau vient d’être réservé. Choisissez-en un autre."}</p> : state === "error" ? <p className="appointment-feedback is-error" role="alert">{ar ? "تعذر تأكيد الموعد. حاول مجددًا." : "Impossible de confirmer le rendez-vous. Réessayez."}</p> : null}
  </section>;
}
