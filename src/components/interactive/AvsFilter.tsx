...
export function AvsFilter({ locale, profiles, cities, filters, page, totalPages, total }: { locale: Locale; profiles: AvsDirectoryProfile[]; cities: string[]; filters: { q?: string; city?: string; certified?: string }; page: number; totalPages: number; total: number }) {
  const ar = locale === "ar";
  return <><form className="avs-search-panel compact-search directory-filter" method="get"><label className="search-field"><Icon name="search" size={19} /><input name="q" maxLength={100} defaultValue={filters.q} placeholder={ar ? "الاسم أو الاختصاص..." : "Nom ou spécialité..."} /></label><select name="city" defaultValue={filters.city ?? ""} aria-label={ar ? "الجهة" : "Ville"}><option value="">{ar ? "كل الجهات" : "Toutes les régions"}</option>{cities.map((item) => <option value={item} key={item}>{item}</option>)}</select><label className="checkbox-filter"><input type="checkbox" name="certified" value="true" defaultChecked={filters.certified === "true"} /><span>{ar ? "معتمد فقط" : "Certifiés uniquement"}</span></label><button className="btn btn-primary" type="submit">{ar ? "بحث" : "Rechercher"}</button><Link className="btn btn-ghost" href={`/${locale}/avs`}>{ar ? "إعادة ضبط" : "Réinitialiser"}</Link></form><div className="catalog-result-summary" role="status">{ar ? `${total} ملف` : `${total} profil${total > 1 ? "s" : ""}`}</div><div className="avs-grid">{profiles.map((profile) => <article className="avs-card premium-card" key={profile.id}><div className="avs-avatar">{profile.initials}</div><div className="avs-card-main"><div className="avs-name-row"><h3>{profile.name}</h3>{profile.certified ? <span className="certified"><Icon name="check" size={13} />{ar ? "معتمد ANEI" : "Certifié ANEI"}</span> : null}</div><p className="avs-specialty">{profile.specialty}</p><div className="meta-row"><span><Icon name="map" size={15} />{profile.city}</span><span className="available-dot">{profile.availability}</span></div></div><Link className="btn btn-secondary" href={`/${locale}/contact?avs=${encodeURIComponent(profile.name)}`}>{ar ? "تواصل" : "Contacter"}</Link></article>)}</div>{!profiles.length ? <div className="empty-state">{ar ? "لا توجد ملفات مطابقة لهذه المعايير." : "Aucun profil ne correspond à ces critères."}</div> : null}<Pagination locale={locale} basePath={`/${locale}/avs`} page={page} totalPages={totalPages} params={filters} /></>;
}
