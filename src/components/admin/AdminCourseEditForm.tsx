...
export function AdminCourseEditForm({ locale, course }: { locale: Locale; course: CourseValues }) {
  ...
  return <form className="admin-course-form" onSubmit={submit}>
    ...
    <button className="btn btn-primary" disabled={state === "loading"}>{state === "loading" ? (ar ? "جارٍ الحفظ..." : "Enregistrement...") : (ar ? "حفظ التعديلات" : "Enregistrer les modifications")}</button>
    {state === "done" ? <small className="success-inline">{ar ? "تم الحفظ." : "Modifications enregistrées."}</small> : state === "error" ? <small className="form-error">{ar ? "تعذر الحفظ." : "Échec de l’enregistrement."}</small> : null}
  </form>;
}
