...
import { createSpecialistAssignment } from "@/server/services/relationships";

const schema = z
  .object({ specialistUserId: z.string().uuid(), studentUserId: z.string().uuid() })
  .strict()
  .refine((data) => data.specialistUserId !== data.studentUserId, { message: "specialistUserId and studentUserId must differ" });
...
export async function POST(request: Request) {
  ...
  try {
    const result = await createSpecialistAssignment(session.user.id, parsed.data.specialistUserId, parsed.data.studentUserId);
