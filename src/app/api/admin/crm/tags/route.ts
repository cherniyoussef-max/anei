...
import { listCrmTags } from "@/server/queries/crm";
import { createCrmTag } from "@/server/services/crm";

const orgQuerySchema = z.object({ organizationId: z.string().uuid() });
const createSchema = z.object({ organizationId: z.string().uuid(), name: z.string().trim().min(1).max(60) }).strict();
...
export async function GET(request: Request) {
  ...
  const tags = await listCrmTags(org.data.organizationId);
...
export async function POST(request: Request) {
  ...
  const result = await createCrmTag(session.user.id, parsed.data.organizationId, parsed.data.name);
