...
import { resolveActorOrgRole, searchAssessments } from "@/server/queries/admission";
import { createAssessment } from "@/server/services/assessments";
...
export async function GET(request: Request) {
  ...
  const result = await searchAssessments(parsed.data);
...
export async function POST(request: Request) {
  ...
  const result = await createAssessment(session.user.id, actorRole, organizationId, data);
  ...
  if (result.kind === "invalid_appointment") return NextResponse.json({ error: "INVALID_APPOINTMENT" }, { status: 422 });
  if (result.kind === "invalid_assessor") return NextResponse.json({ error: "INVALID_ASSESSOR" }, { status: 422 });
