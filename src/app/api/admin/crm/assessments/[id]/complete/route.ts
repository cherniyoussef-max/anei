...
import { completeAssessment } from "@/server/services/assessments";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await completeAssessment(session.user.id, actorRole, organizationId, id.data);
  ...
  if (result.kind !== "ok") return NextResponse.json({ error: "COMPLETE_FAILED" }, { status: 409 });
