...
import { revokeAccountInvitation } from "@/server/services/account-invitations";

const idSchema = z.string().uuid();
const bodySchema = z.object({ organizationId: z.string().uuid() }).strict();

/** Revokes an invitation. REVOKED is terminal — any outstanding token/OTP for it stops working. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await revokeAccountInvitation(session.user.id, actorRole, parsed.data.organizationId, id.data);
