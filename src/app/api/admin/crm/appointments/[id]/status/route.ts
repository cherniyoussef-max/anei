...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await setAppointmentStatus(session.user.id, actorRole, organizationId, id.data, status);
  ...
  if (result.kind !== "ok") return NextResponse.json({ error: "STATUS_FAILED" }, { status: 409 });
