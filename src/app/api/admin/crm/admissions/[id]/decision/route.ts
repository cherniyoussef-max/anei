...
import { decideAdmission } from "@/server/services/admissions";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await decideAdmission(session.user.id, actorRole, organizationId, id.data, decision, reason);
  ...
  if (result.kind !== "ok") return NextResponse.json({ error: "DECISION_FAILED" }, { status: 409 });
