  ...
  return <div className="course-room">
    <header className="course-room-hero" id="overview">
      <div className="course-room-hero-copy">
        <Link className="course-room-back" href={`/${locale}/dashboard/formations`}><Icon className="course-room-back-icon" name="arrow" size={17}/>{c.back}</Link>
        <p className="course-room-context">{c.room}</p>
        <h1>{title}</h1>
        <p className="course-room-lead">{description}</p>
        <div className="course-room-meta"><span><Icon name="clock" size={17}/>{Math.max(1, Math.round(data.course.durationMinutes / 60))} {c.hours}</span><span><Icon name="user" size={17}/>{c.trainer}: {data.course.trainerName}</span></div>
        {firstIncomplete ? <Link className="course-room-primary" href={lessonHref(firstIncomplete.id)}><Icon name="play" size={17}/>{c.continue}</Link> : null}
      </div>
